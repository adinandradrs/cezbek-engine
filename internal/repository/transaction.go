...
type TransactionPersister interface {
	Add(trx model.Transaction) *model.TechnicalError
}
...
func (t *Transaction) Add(trx model.Transaction) *model.TechnicalError {
	tx, err := t.Pool.BeginTx(context.Background(),
		pgx.TxOptions{IsoLevel: pgx.Serializable})
	if err != nil {
		return apps.Exception("failed to begin add kezbek tx", err, zap.Any("", trx), t.Logger)
	}
	...
	_, err = tx.Exec(context.Background(), `INSERT INTO transactions 
		...
		trx.CreatedBy.Int64)
	if err != nil {
		return apps.Exception("failed to add kezbek tx", err, zap.Any("", trx), t.Logger)
	}
